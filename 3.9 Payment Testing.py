...
def process_attendance_payroll(
    ...
    try:
        ...
        # 动态获取列位置 - SG (基于最终列顺序)
        def get_sg_column_positions():
            # 基于重新排列后的列顺序
            columns_order = ['Month of billing', 'Employee ID', '1BankID', 'Contractor Name', 'Billing PC Code', 'BU', 'DBS Manager Name', 'Join Date', 'Exit Date', 'Rate Type', 'Bill Rate', 'No. of days', 'Cost(RMB)', 'Tax (RMB)', 'Total (RMB)', 'Assignment ID', 'NCS Value', 'Validation']
            rate_type_col = excel_column(columns_order.index('Rate Type') + 1)
            bill_rate_col = excel_column(columns_order.index('Bill Rate') + 1) 
            actual_days_col = excel_column(columns_order.index('No. of days') + 1)
            cost_col = excel_column(columns_order.index('Cost(RMB)') + 1)
            tax_col = excel_column(columns_order.index('Tax (RMB)') + 1)
            total_col = excel_column(columns_order.index('Total (RMB)') + 1)
            ncs_value_col = excel_column(columns_order.index('NCS Value') + 1)
            return rate_type_col, bill_rate_col, actual_days_col, cost_col, tax_col, total_col, ncs_value_col

        rate_type_col, bill_rate_col, actual_days_col, cost_col, tax_col, total_col, ncs_value_col = get_sg_column_positions()

        # 计算 'Cost(RMB)' - 使用Excel公式
        mother_table['Cost(RMB)'] = mother_table.apply(lambda row: 
            f'=IF({rate_type_col}{row.name+5}="Monthly",'
            f'({actual_days_col}{row.name+5}/{standard_days})*{bill_rate_col}{row.name+5},'
            f'IF({rate_type_col}{row.name+5}="Daily",'
            f'{actual_days_col}{row.name+5}*{bill_rate_col}{row.name+5},0))', axis=1)

        # 计算 'Tax (RMB)' - 使用Excel公式
        mother_table['Tax (RMB)'] = mother_table.apply(lambda row:
            f'=IF({rate_type_col}{row.name+5}="Monthly",{cost_col}{row.name+5}*0.06,0)', axis=1)

        # 计算 'Total (RMB)' - 使用Excel公式
        mother_table['Total (RMB)'] = mother_table.apply(lambda row:
        ...
        # 只保留NCS Value的数值转换（删除Total的数值转换）：
        mother_table['NCS Value'] = pd.to_numeric(mother_table['NCS Value'], errors='coerce')
        # 注释掉：mother_table['Total (RMB)'] = pd.to_numeric(mother_table['Total (RMB)'], errors='coerce')

        # 创建 'Validation' 列 - 使用Excel公式
        mother_table['Validation'] = mother_table.apply(lambda row:
            f'=IF(ABS({ncs_value_col}{row.name+5}-{total_col}{row.name+5})<=0.01,"True","False")', axis=1)
    ...
    try:
        ...
        for row in range(5, ws_sg.max_row + 1):  # 从第5行开始，因为数据从第4行开始
            ...
            if df_row_idx < len(mother_table):
                try:
                    ncs_value = mother_table.iloc[df_row_idx]['NCS Value']
                    actual_days = mother_table.iloc[df_row_idx]['No. of days']
                    bill_rate = mother_table.iloc[df_row_idx]['Bill Rate']
                    rate_type = mother_table.iloc[df_row_idx]['Rate Type']
    ...
    try:
        ...
        rate_type_col_cn, bill_rate_col_cn, actual_days_col_cn, cost_col_cn, tax_col_cn, total_col_cn, ncs_value_col_cn = get_cn_column_positions()

        # 计算 'Cost(RMB)' - 使用Excel公式
        cn_table['Cost(RMB)'] = cn_table.apply(lambda row: 
            f'=IF(OR({rate_type_col_cn}{row.name+5}="Monthly",{rate_type_col_cn}{row.name+5}="monthly"),'
            f'({actual_days_col_cn}{row.name+5}/{standard_days})*{bill_rate_col_cn}{row.name+5},'
            f'IF(OR({rate_type_col_cn}{row.name+5}="Daily",{rate_type_col_cn}{row.name+5}="daily"),'
            f'{actual_days_col_cn}{row.name+5}*{bill_rate_col_cn}{row.name+5},0))', axis=1)

        # 计算 'Tax (RMB)' - 使用Excel公式
        cn_table['Tax (RMB)'] = cn_table.apply(lambda row:
            f'=IF({rate_type_col_cn}{row.name+5}="Monthly",{cost_col_cn}{row.name+5}*0.06,0)', axis=1)

        # 计算 'Total (RMB)' - 使用Excel公式
        cn_table['Total (RMB)'] = cn_table.apply(lambda row:
            f'=ROUND({cost_col_cn}{row.name+5}+{tax_col_cn}{row.name+5},2)', axis=1)
        ...
        # 注释掉：cn_table['Total (RMB)'] = pd.to_numeric(cn_table['Total (RMB)'], errors='coerce')

        # 创建 'Validation' 列 - 使用Excel公式
        cn_table['Validation'] = cn_table.apply(lambda row:
            f'=IF(ABS({ncs_value_col_cn}{row.name+5}-{total_col_cn}{row.name+5})<=0.01,"True","False")', axis=1)
    ...
    try:
        ...
        write_dataframe_to_sheet(ws_cn, cn_table, start_row=4, start_col=1)
        # 插入内容到K3/L3单元格
        ws_cn['K3'] = 'Acutal Working Days'
        ws_cn['L3'] = standard_days
        add_border_to_cell(ws_cn['K3'])
        add_border_to_cell(ws_cn['L3'])
        # 为新添加的单元格添加边框
        add_border_to_cell(ws_cn['M3'])
        add_border_to_cell(ws_cn['N3'])
        add_border_to_cell(ws_cn['O3'])
        add_border_to_cell(ws_cn['Q3'])
        # 使用write_dataframe_to_sheet函数写入cn_table数据
        write_dataframe_to_sheet(ws_cn, cn_table, start_row=4, start_col=1)
        # 在这里插入新的格式化代码
        # 设置A4-R4的样式
        for col in range(1, 19):  # A到R列
            cell = ws_cn.cell(row=4, column=col)
        ...
        # 将DataFrame写入Excel，从第4行A列开始
        for r_idx, row in enumerate(dataframe_to_rows(cn_table, index=False, header=True), 4):
            for c_idx, value in enumerate(row, 1):
             ws_cn.cell(row=r_idx, column=c_idx, value=value)
        
        # 获取 'Validation' 列的索引 和  # 获取 '1BankID' 列的索引
        validation_col = cn_table.columns.get_loc('Validation') + 1
        bank_id_col = cn_table.columns.get_loc('1BankID') + 1
        # 创建黄色填充样式和红色字体样式
        yellow_fill = PatternFill(start_color='FFFFFF00', end_color='FFFFFF00', fill_type='solid')
        red_font = Font(color='FF0000')
        # 用于跟踪重复的 1BankID
        bank_id_count = {}
        # 遍历所有行（跳过标题行）
        for row in range(5, ws_cn.max_row + 1):  # 从第5行开始，因为数据从第4行开始
            df_row_idx = row - 5
            
            # 检查 Validation 列 - 基于实际数据比较
            if df_row_idx < len(cn_table):
                try:
                    ncs_value = cn_table.iloc[df_row_idx]['NCS Value']
                    actual_days = cn_table.iloc[df_row_idx]['No. of days']
                    bill_rate = cn_table.iloc[df_row_idx]['Bill Rate']
                    rate_type = cn_table.iloc[df_row_idx]['Rate Type']
                    
                    # 重新计算Cost和Total来比较
                    if rate_type in ['Monthly', 'monthly']:
                        cost = (actual_days / standard_days) * bill_rate if not pd.isna(bill_rate) else 0
                        tax = cost * 0.06
                    elif rate_type in ['Daily', 'daily']:
                        cost = actual_days * bill_rate if not pd.isna(bill_rate) else 0
                        tax = 0
                    else:
                        cost = 0
                    ...
                    if not pd.isna(ncs_value) and abs(ncs_value - calculated_total) > 0.01:
                        validation_cell = ws_cn.cell(row=row, column=validation_col)
                        validation_cell.font = red_font
            ...
            # 检查 '1BankID' 列（保持不变）
            bank_id_cell = ws_cn.cell(row=row, column=bank_id_col)
            bank_id = bank_id_cell.value
            if bank_id in bank_id_count:
                bank_id_cell.fill = yellow_fill
                ws_cn.cell(row=bank_id_count[bank_id], column=bank_id_col).fill = yellow_fill
            else:
                bank_id_count[bank_id] = row
    ...
    try:
        ...
        bill_rate_mapping = {
            ...
            ('Generic', 'Lead Consultant', 'Monthly'): 62376,
            ...
            ('Niche', 'Lead Consultant', 'Monthly'): 62376,
        ...
        column_positions = {
            'actual_days': 'P',  # 假设 'No. of days' 在 P 列
            'bill_rate': 'O',    # 假设 'Rate per month(RMB)' 在 O 列
            'rate_type': 'N',    # 假设 'Rate Type' 在 N 列
            'cost': 'Q',         # 假设 'Cost(RMB)' 在 Q 列
            'tax': 'R',          # 假设 'Tax (RMB)' 在 R 列
            'total': 'S',        # 假设 'Total (RMB)' 在 S 列
            'ncs_value': 'U'     # 假设 'NCS Value' 在 U 列
}

        # 计算 'Cost(RMB)'
        hk_table['Cost(RMB)'] = hk_table.apply(lambda row: 
            f'=IF(OR({column_positions["rate_type"]}{row.name+5}="Monthly",{column_positions["rate_type"]}{row.name+5}="Daily"),'
            f'IF({column_positions["rate_type"]}{row.name+5}="Monthly",'
            f'({column_positions["actual_days"]}{row.name+5}/{standard_days})*{column_positions["bill_rate"]}{row.name+5},'
            f'{column_positions["actual_days"]}{row.name+5}*{column_positions["bill_rate"]}{row.name+5}),0)', axis=1)

        # 计算 'Tax (RMB)'
        hk_table['Tax (RMB)'] = hk_table.apply(lambda row:
            f'=IF({column_positions["rate_type"]}{row.name+5}="Monthly",{column_positions["cost"]}{row.name+5}*0.06,0)', axis=1)

        # 计算 'Total (RMB)'
        hk_table['Total (RMB)'] = hk_table.apply(lambda row:
            f'=ROUND({column_positions["cost"]}{row.name+5}+{column_positions["tax"]}{row.name+5},2)', axis=1)

        # 添加Validation公式
        hk_table['Validation'] = hk_table.apply(lambda row:
            f'=IF({column_positions["ncs_value"]}{row.name+5}={column_positions["total"]}{row.name+5},"True","False")', axis=1)


        # 重新排列列顺序
        hk_table = hk_table[[
           'Employee ID', 'Contract No.','1BankID', 'Contractor Name', 'Specialization','Roles','Billing PC Code','location','Perm/Vendor','BU','Original Start Date', 'Contract End Date', 'Rate Type','Rate per month(RMB)','No. of days','Cost(RMB)','Tax (RMB)','Total (RMB)','Assignment ID','Total amt(incl. VAT)', 'Validation'
        ]]
        hk_table.insert(0,'Month of billing',billing_month)

        print(hk_table.columns)
        ...
        for row in range(5, ws_hk.max_row + 1):  # 从第5行开始，因为数据从第4行开始
            ...
            if df_row_idx < len(hk_table):
                try:
                    ...
                    # 确保数据类型正确
                    ncs_value = float(ncs_value) if not pd.isna(ncs_value) else None
                    actual_days = float(actual_days) if not pd.isna(actual_days) else 0
                    bill_rate = float(bill_rate) if not pd.isna(bill_rate) else 0
                    ...
                    if ncs_value is not None and calculated_total is not None:
                        if abs(ncs_value - calculated_total) > 0.001:  # 使用更小的容差
                            validation_cell = ws_hk.cell(row=row, column=validation_col)
                            validation_cell.font = red_font
                        else:
                            validation_cell = ws_hk.cell(row=row, column=validation_col)
                            validation_cell.font = Font(color="00000000")  # 黑色字体
                    else:
                        # 如果任一值为None，标记为需要检查
                        validation_cell = ws_hk.cell(row=row, column=validation_col)
                        validation_cell.font = red_font
    ...
    for row in range(5, ws.max_row + 1):
        ...
        if df_row_idx < len(table):
            try:
                ...
                bill_rate = table.iloc[df_row_idx]['Rate per month(RMB)'] if 'Rate per month(RMB)' in table.columns else 0
